#!/usr/bin/env python3
"""
Apply Learned Patterns for {{agent_name}}
...
def preview_improvements() -> Dict:
    ...
    for pattern in patterns:
        ...
        if pattern.get("type") == "context_learned":
            improvement["would_modify"] = [f"Apply {pattern.get('tool')} best practices"]
        elif pattern.get("type") == "correction":
            improvement["would_modify"] = ["Check similar issues in component"]
        elif pattern.get("type") == "workflow":
            improvement["would_modify"] = ["May add to workflows"]
        elif pattern.get("type") == "antipattern":
            improvement["would_modify"] = ["Add to antipatterns reference"]
